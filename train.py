...
def train(hyp, opt, device, callbacks):
        ...
        for opt_key in opt.__dict__:
          ...
          if opt_value is not None and opt_value!='' and opt_key=='data':
             mlflow.log_artifact(opt_value,'data.yaml')
        ...
        mlflow.log_artifact('yolo_model_summary.txt',artifact_path='model_info')
        os.remove('yolo_model_summary.txt')
        ...
        for epoch in range(
            ...
            for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                ...
                mlflow.log_metric(tag, float(x),step=epoch)
            scheduler.step()
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        if opt.bucket:
            subprocess.run(
                f"gsutil cp gs://{opt.bucket}/evolve.csv {evolve_csv}".split()
